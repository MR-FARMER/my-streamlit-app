...
# ===== 语法定义与关键字（新增） =====
group_syntax = {
    "定义一个函数或方法（可执行语句）": "def 函数名(参数):",
    "创建匿名函数（表达式，返回函数对象）": "lambda 参数: 返回值",
    "从函数中返回值（结束函数执行）": "return 值",
    "生成一个生成器（迭代器）": "yield 值",
    "定义一个类（面向对象模板）": "class 类名:",
    "导入模块或包（引入外部代码）": "import 模块名",
    "从模块中导入指定属性或函数": "from 模块 import 名称",
    "用 as 给导入的模块起别名": "import 模块 as 别名",
    "检查条件是否成立，成立则执行代码块": "if 条件:",
    "条件不成立时执行另一个代码块": "else:",
    "在上一个条件不成立时检查新条件": "elif 条件:",
    "循环遍历可迭代对象": "for 变量 in 可迭代对象:",
    "条件为真时循环执行代码块": "while 条件:",
    "跳出当前循环（for/while）": "break",
    "跳过当前迭代，进入下一次循环": "continue",
    "什么也不做（占位符）": "pass",
    "删除变量或对象引用": "del 变量名",
    "断言条件为真，否则抛出异常": "assert 条件",
    "全局变量声明（在函数内修改全局变量）": "global 变量名",
    "非局部变量声明（在嵌套函数内修改外层变量）": "nonlocal 变量名",
}
